...
export const Nav = () => {
    ...
    return (
        <>
            <nav className="bg-secondaryBlack w-full border-b-2 border-tertiary fixed z-50 h-20">
                <motion.div className="max-w-7xl mx-auto px-4 md:px-8 h-full flex flex-row items-center justify-between" initial={{ opacity: 0, scale: 0.9 }}
                    ...
                    <ul className="hidden md:flex flex-row justify-center items-center gap-8">
                        {navItems.map((item) => (
                            <li key={item.id}>
                                <a href={item.path} className="font-medium hover:text-primary text-secondary transition-colors hover:underline hover:underline-offset-4">{item.label}</a>
                            </li>
                        ))}
                    </ul>
                    <button className="hidden md:flex flex-row items-center justify-center gap-2 bg-primary text-secondary px-6 py-2 rounded-2xl shadow-xl hover:bg-secondary hover:text-secondaryBlack transition-colors">Empezá tu proyecto</button>
