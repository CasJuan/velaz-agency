...
export const CardServices = ({ icon, title, desc, items, btn }) => {
    ...
    return (
        <div className="flex flex-col items-start justify-center gap-5 h-auto w-80 bg-white rounded-2xl shadow p-5 border border-transparent transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:border-primary/20 group cursor-pointer">
            ...
            <h2 className="text-2xl font-bold transition-colors duration-300 group-hover:text-secondary">
                {title}
            </h2>
            <p className="text-lg text-gray-500">{desc}</p>
            <ul className="flex flex-col items-start justify-center gap-1">
                {items.map((item) => (
                    <li key={item} className="flex flex-row items-center gap-2 text-sm">
                        <Check color="#730000" className="w-4 shrink-0" />
                        {item}
                    </li>
                ))}
            </ul>
            <button className="flex flex-row items-center justify-center gap-2 bg-tertiary text-secondary rounded-2xl shadow-2xl h-auto w-50 p-1 self-center transition-all duration-300 group-hover:bg-secondary group-hover:text-white">
